...
public interface MottoRepository extends
        JpaRepository<MottoEntity, UUID>,
        RevisionRepository<MottoEntity, UUID, Integer> {
    @Query("""
        SELECT e FROM MottoEntity e WHERE
    ...
    """)
    Page<MottoEntity> filter(FilterPayload payload,
