...
async function handleFilter() {
    ...
    let url = `/v1/api/category-best-food/filter?categoryPageId=${location.pathname.split("/")[2]}&pageSize=${pageSize}&page=${currentPage}`;
    await callApi(url, {
        method: 'POST',
        ...
        const promises = data.data.data.map(item =>
            callApi("/v1/api/food/" + item.foodId, {method: 'GET'}));

        await Promise.allSettled(promises).then((results) => {
            results.forEach((res, idx) => {
                if (res.status === "fulfilled") {
                    data.data.data[idx].foodName = res.value.data.name;
                } else {
                    data.data.data[idx].foodName = "-";
                }
            });
        });
        ...
        tbody.innerHTML = data.data.data.map((item, index) =>
            `
                <tr class="cursor-pointer" onclick="viewDetail('${item.id}')">
                    ...
                    <td><div class="text-truncate" style="min-width: 100px; max-width: 200px;" data-bs-toggle="tooltip">${item.foodName}</div></td>
