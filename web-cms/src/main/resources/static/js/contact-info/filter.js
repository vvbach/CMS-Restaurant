...
async function handleFilter(e) {
    ...
    let url = "/v1/api/contact-info/filter?pageSize=" + pageSize + "&page=" + currentPage;
    await callApi(url, {
        method: 'POST',
        ...
        tbody.innerHTML = data.data.data.map((item, index) =>
            `
                <tr onclick="viewDetail('${item.id}')">
                    ...
                    <td><div class="text-truncate" style="min-width: 100px; max-width: 200px;" data-bs-toggle="tooltip">${item.text}</div></td>
